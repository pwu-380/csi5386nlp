...
def build_model(tparams, options):
    ...
    probs = tensor.nnet.relu(logit)
    cost = theano.tensor.square(probs.reshape((n_samples,)) - y)

    f_pred = theano.function([x1, x1_mask, x2, x2_mask], probs.reshape((n_samples,)), name='f_pred')
...
def pred_acc(f_pred, prepare_data, options, iterator, verbose=False, print_pred=False):
    """
    ...
    for x1, x2, y in iterator:
        ...
        pearson_corr = ((preds - pred_avg)*(y - y_avg)).sum() / (numpy.sqrt(((preds - pred_avg) ** 2).sum()) *
        numpy.sqrt(((y - y_avg) ** 2).sum()))

        valid_mse += ((preds - y) ** 2).sum()
        valid_acc += pearson_corr * n

        if print_pred:
            for pred in preds:
                print pred

    valid_acc = 1.0 * valid_acc / n_done        #Weighted average pearson correlation by batch
    valid_mse = 1.0 * valid_mse / n_done        #MSE
...
def train(dim_word=100,  # word vector dimensionality
    ...
    for eidx in xrange(max_epochs):
        ...
        for x1, x2, y in train:
            ...
            if numpy.mod(uidx, validFreq) == 0:
                ...
                valid_acc, valid_mse = pred_acc(f_pred, prepare_data,
                                        model_options, valid)
                ...
                test_acc, test_mse = pred_acc(f_pred, prepare_data,
                                        model_options, test)
    ...
    train_acc, train_mse = pred_acc(f_pred, prepare_data,
    ...
    test_cost = pred_probs(f_log_probs, prepare_data,
                           model_options, test).mean()
    test_acc, test_mse = pred_acc(f_pred, prepare_data,
                         model_options, test, print_pred=True)
